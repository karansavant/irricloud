...
//$$NR$$//dated:6Feb16
USHORT  mb_ippressure_curvalue = 0;
USHORT  mb_oppressure_curvalue = 0;
USHORT  mb_diffpressure_curvalue = 0;
USHORT  mb_temperature_curvalue = 0;
USHORT  mb_humidity_curvalue = 0;
USHORT  mb_sensor_errors = 0;

UCHAR   mb_rain_curvalue = 0;
UCHAR   mb_sensor_error_bit = 0;
UCHAR   mb_limit_status_on_dP = 0;
//$$NR$$//dated:6Feb16
...
//$$NR$$//dated:7Feb16
            ...
            && (usAddress + usNRegs <= REG_INPUT_START + REG_INPUT_NREGS))
    {
        ...
        while (usNRegs > 0)
        {
            ...
            if (iRegIndex == 0)
                mb_ippressure_curvalue  = pusRegInputBuf[0];
            else if (iRegIndex == 1)
                mb_oppressure_curvalue  = pusRegInputBuf[1];
            else if (iRegIndex == 2)
                mb_diffpressure_curvalue= pusRegInputBuf[2];
            else if (iRegIndex == 3)
                mb_temperature_curvalue = pusRegInputBuf[3];
            else if (iRegIndex == 4)
                mb_humidity_curvalue    = pusRegInputBuf[4];
            else if (iRegIndex == 5)
                mb_sensor_errors        = pusRegInputBuf[5];
...
eMBErrorCode eMBMasterRegDiscreteCB( UCHAR * pucRegBuffer, USHORT usAddress, USHORT usNDiscrete )
{
            ...
            && (usAddress + usNDiscrete    <= DISCRETE_INPUT_START + DISCRETE_INPUT_NDISCRETES))
    {
        ...
        mb_rain_curvalue        = pucDiscreteInputBuf[0];
        mb_sensor_error_bit     = pucDiscreteInputBuf[1];
        mb_limit_status_on_dP   = pucDiscreteInputBuf[2];
